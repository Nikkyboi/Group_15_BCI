...
if __name__ == "__main__":
    ...
    # train on all except held_out, test on held_out
    train_idx = (groups != held_out).nonzero(as_tuple=True)[0].numpy()
    ...
    plt.savefig(reports_dir / f"training_validation_loss_{held_out_name}_{model_name}.png")
    plt.close()
    ...
    plt.figure()
    plt.plot(y_true.numpy(), label="True", marker="o")
    plt.plot(y_pred.numpy(), label="Pred", marker="x")
    plt.xlabel("Test trial index")
    plt.ylabel("Class")
    plt.title(f"True vs Predicted - {held_out_name}")
    plt.legend()
    plt.savefig(reports_dir / f"test_true_vs_pred_{held_out_name}_{model_name}.png")
    plt.close()